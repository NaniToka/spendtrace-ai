...
class BillingItemSchema(BaseModel):
    ...
    usage_quantity: float = Field(..., ge=0.0, description="Usage amount")
    unit_cost: float = Field(..., ge=0.0, description="Unit cost rate in USD")
    total_cost: float = Field(..., ge=0.0, description="Total billed cost in USD")
